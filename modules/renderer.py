...
class Camera(object):
    ...
    def _render_floor_and_ceiling(self: Self,
        ...
        if difference >= 1:
            ...
            texture_ys = np.floor(y_points / scale[1] % 1 * texture.height)
    ...
    def _render_walls_and_entities(self: Self,
        ...
        for x in range(width):
            ...
            path = ray * self._wall_render_distance / mag
            end_pos = self._player._pos - path
            slope = ray.y / ray.x if ray.x else math.inf
            tile = pg.Vector2(math.floor(end_pos.x), math.floor(end_pos.y))
            dir = (ray.x > 0, ray.y > 0)
            depth = self._wall_render_distance / mag
            dist = 0
            tile_key = None
